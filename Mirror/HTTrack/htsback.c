            ...
            if (!back[p].testmode) {  // not test mode
                ...
                if (back[p].r.compressed) {
                    if (back[p].r.size > 0) {
                        ...
                        // stats
                        // MIRROR_ADAPTATION START : Changed global variables to point to structure
                        opt->stats.total_packed += back[p].compressed_size;
                        opt->stats.total_unpacked += back[p].r.size;
                        opt->stats.total_packedfiles++;
                        // MIRROR_ADAPTATION END : Changed global variables to point to structure
                        // unflag
                    }
                }
#endif
                ...
                if (back[p].r.statuscode == HTTP_OK) {
                    if (back[p].r.size >= 0) {
                        ...
                        if ((!back[p].r.notmodified) && (opt->is_update)) {
                            opt->stats.stat_updated_files++;    // page modifiée
                            // MIRROR_ADAPTATION END : Changed global variables to point to structure
                            if (back[p].is_update) {
                                hts_log_print(opt, LOG_INFO,
...
int back_serialize_ref(httrackp * opt, const lien_back * src) {
    ...
    if (fp == NULL) {
#ifdef _WIN32
        if (mkdir(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_REFNAME)) == 0)
#else
            if (mkdir(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_REFNAME), S_IRWXU | S_IRWXG | S_IRWXO) == 0)
#endif
            {
                /* note: local filename */
...
int back_add_if_not_exists(struct_back * sback, httrackp * opt,
                           ...
                           unsigned long base_id) { // MIROIR_ADAPTATION : Added parameter 'base_id'
...
int back_add(struct_back * sback, httrackp * opt, cache_back * cache, const char *adr,
             ...
             int test, unsigned long base_id) { // MIROIR_ADAPTATION : Added parameter 'base_id'
    lien_back *const back = sback->lnk;
    const int back_max = sback->count;
    int p = 0;
    char catbuff[CATBUFF_SIZE];
    ...
    if ((p = back_search(opt, sback)) >= 0) {
        ...
        // créer entrée
        back[p].base_id = base_id; // MIRROR_ADAPTATION : Set up reference to base link identifier
...
#error Must have pthreads
                ...
                for(i = 0; i < back_max; i++) {
                    if (slot_can_be_cleaned(&back[i])) {
                        if (slot_can_be_finalized(opt, &back[i])) {
                            ...
                            usercommand(opt, 0, NULL, back[i].url_sav, back[i].url_adr, back[i].url_fil, back[i].r.size); // MIRROR_ADAPTATION : Added size parameter
                            ...
                            opt->stats.stat_background++; // MIRROR_ADAPTATION : Changed global variables to point to structure
                            hts_log_print(opt, LOG_INFO,
            ...
            void back_wait(struct_back * sback, httrackp * opt, cache_back * cache, TStamp stat_timestart) {
                ...
                (unsigned int) (time_local() + opt->stats.HTS_TOTAL_RECV); // MIRROR_ADAPTATION : Changed global variables to point to structure
                        ...
                        if (back[i].status == STATUS_CONNECTING) {        // attendre connect
                            ...
                            if (dispo) {            // ok connected!!
                                ...
                                if ((back[i].r.soc != INVALID_SOCKET)
                                    ...
                                    opt->stats.stat_nrequests++; // MIRROR_ADAPTATION : Changed global variables to point to structure
                            ...
                            if (dispo) {            // données dispo
                                ...
                                if (back[i].status) {
                                    if (back[i].status == STATUS_WAIT_HEADERS)  // recevoir par bloc de lignes
                                        // MIRROR_ADAPTATION : Added back[i].base_id argument
                                        retour_fread = http_xfread1(&(back[i].r), 0, opt, back[i].base_id);
                                        ...
                                        // ligne par ligne
                                        // MIRROR_ADAPTATION : Added back[i].base_id argument
                                        retour_fread = http_xfread1(&(back[i].r), -1, opt, back[i].base_id);
...
#endif
                                        // MIRROR_ADAPTATION : Added back[i].base_id argument
                                        retour_fread = (int)http_xfread1(&(back[i].r), (int) min(back[i].r.totalsize - back[i].r.size, max_read_bytes), opt, back[i].base_id);
                                    } else
                                        retour_fread = (int)http_xfread1(&(back[i].r), (int) max_read_bytes, opt, back[i].base_id); // MIRROR_ADAPTATION : Added back[i].base_id argument
                                } else
                                                ...
                                                if (back[i].status != 0 && back[i].r.adr[0] != '<') {
                                                    ...
                                                    if (opt->debug_head) {
                                                        if (opt->ioinfo) {
                                                            ...
                                                            fprintfio(opt->ioinfo, back[i].r.adr, ">>> ");
                                                            fprintf(opt->ioinfo, "\r\n");
                                                            fflush(opt->ioinfo);
                                                        }         // en-tête
                                                    }
                                                        ...
                                                        if (back[i].r.statuscode == HTTP_OK && !back[i].testmode) {       // 'OK'
                                                            if (!is_hypertext_mime(opt, back[i].r.contenttype, back[i].url_fil)) {  // not HTML
                                                                if (strnotempty(back[i].url_sav)) {   // target found
                                                                    int size = (int)fsize_utf8(back[i].url_sav);     // target size
            ...
            int back_checkmirror(httrackp * opt) {
                // Check max size
                if ((opt->maxsite > 0) && (opt->stats.stat_bytes >= opt->maxsite)) { // MIRROR_ADAPTATION : Changed global variables to point to structure
                ...
                // Check max time
                if ((opt->maxtime > 0)
                    && ((time_local() - opt->stats.stat_timestart) >= opt->maxtime)) { // MIRROR_ADAPTATION : Changed global variables to point to structure
                    if (!opt->state.stop) {     /* not yet stopped */
