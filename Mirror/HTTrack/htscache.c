                            ...
                            if (coucal_read
                                (cache->cached_tests,
                                 concat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), url_adr, url_fil), NULL) == 0) {
                                    ...
                                    coucal_add(cache->cached_tests, concat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), url_adr, url_fil), (intptr_t) strdupt(tempo));
                                }
                        }
                    }
                }
...
void cache_add(httrackp * opt, cache_back * cache, const htsblk * r, const char *url_adr, const char *url_fil, const char *url_save, int all_in_cache, const char *path_prefix) {
    ...
    if (url_save == NULL) {
        ...
        /* Data in cache ? */
        if (is_hypertext_mime(opt, r->contenttype, url_fil) || (may_be_hypertext_mime(opt, r->contenttype, url_fil) && r->adr != NULL) /* store error messages! */ || !HTTP_IS_OK(r->statuscode)) {
            dataincache = 1;
...
static htsblk cache_readex_old(httrackp * opt, cache_back * cache, const char *adr, const char *fil, const char *save, char *location, char *return_save, int readonly);

static htsblk cache_readex_new(httrackp * opt, cache_back * cache, const char *adr, const char *fil, const char *save, char *location, char *return_save, int readonly);
...
// lecture d'un fichier dans le cache
// si save==null alors test unqiquement
static htsblk cache_readex_new(httrackp * opt, cache_back * cache, const char *adr, const char *fil, const char *target_save, char *location, char *return_save, int readonly) {
    ...
    void cache_init(cache_back * cache, httrackp * opt) {
        ...
        hts_log_print(opt, LOG_DEBUG, "Cache: enabled=%d, base=%s, ro=%d",
                      (int) opt->cache, fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                ...
                                                CACHE_DIRECTORY_NAME"/"), (int) cache->ro);
        if (opt->cache) {
#if DEBUGCA
            printf("cache init: ");
#endif
            if (!cache->ro) {
#ifdef _WIN32
                mkdir(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME));
#else
                mkdir(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME),
                      HTS_PROTECT_FOLDER);
#endif
                if ((fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.zip")))) {       // il existe déja un cache précédent.. renommer
                    ...
                    if ((fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.dat"))) && (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.ndx")))) {     // il existe déja un cache précédent.. renommer
                        rename(fconcat
                               (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                CACHE_DIRECTORY_NAME"/new.dat"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                         StringBuff(opt->path_log),
                                                                         CACHE_DIRECTORY_NAME"/old.dat"));
                        rename(fconcat
                               (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                CACHE_DIRECTORY_NAME"/new.ndx"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                         StringBuff(opt->path_log),
                                                                         CACHE_DIRECTORY_NAME"/old.ndx"));
                    }
                         ...
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/old.zip"))) {
                             if (remove
                                 (fconcat
                                  (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                   CACHE_DIRECTORY_NAME"/old.zip")) != 0) {
                    ...
                    if (hts_rename
                        (opt,
                         fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                 CACHE_DIRECTORY_NAME"/new.zip"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                          StringBuff(opt->path_log),
                                                                          CACHE_DIRECTORY_NAME"/old.zip")) != 0) {
                             hts_log_print(opt, LOG_WARNING | LOG_ERRNO,
                                           "Cache: error while moving previous cache");
                ...
                } else if ((fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.dat"))) && (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.ndx")))) {        // il existe déja un cache précédent.. renommer
#if DEBUGCA
            ...
            if ((!cache->ro
                ...
                || (cache->ro
                    ...
                    fsize(fconcat
                          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                           CACHE_DIRECTORY_NAME"/new.zip")) > 0)
                ) {
                if (!cache->ro) {
                    cache->zipInput =
                    unzOpen(fconcat
                ...
                if (cache->zipInput == NULL && !cache->ro) {
                    ...
                    if (unzRepair
                        (name,
                         fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                 CACHE_DIRECTORY_NAME"/repair.zip"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                             StringBuff(opt->path_log),
                                                                             CACHE_DIRECTORY_NAME"/repair.tmp"),
                ...
                if ((!cache->ro
                     ...
                     fsize(fconcat
                           (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                            CACHE_DIRECTORY_NAME"/old.dat")) >= 0
                     &&
                     fsize(fconcat
                           (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                            CACHE_DIRECTORY_NAME"/old.ndx")) > 0)
                    || (cache->ro
                        &&
                        fsize(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.dat")) >= 0
                        &&
                        fsize(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.ndx")) > 0)
                    ) {
                    FILE *oldndx = NULL;
                    ...
                    if (!cache->ro) {
                        ...
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/old.dat"), "rb");
                        oldndx =
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/old.ndx"), "rb");
                    } else {
                        cache->olddat =
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.dat"), "rb");
                        oldndx =
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.ndx"), "rb");
                    }
                ...
                if (1) {
                    ...
                    if (cache->zipOutput != NULL) {
                        ...
                        if (fexist
                            ...
                            rename(fconcat
                                   (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                    CACHE_DIRECTORY_NAME"/new.lst"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                             StringBuff(opt->path_log),
                                                                             CACHE_DIRECTORY_NAME"/old.lst"));
                        // ouvrir
                        cache->lst =
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.lst"), "wb");
                        ...
                        if (fexist
                            ...
                            rename(fconcat
                                   (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                                    CACHE_DIRECTORY_NAME"/new.txt"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                             StringBuff(opt->path_log),
                                                                             CACHE_DIRECTORY_NAME"/old.txt"));
                        // ouvrir
                        cache->txt =
                        fopen(fconcat
                              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/new.txt"), "wb");
                        if (cache->txt) {
                            fprintf(cache->txt,
                    ...
                    fopen(fconcat
                          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                           CACHE_DIRECTORY_NAME"/new.dat"), "wb");
                    cache->ndx =
                    fopen(fconcat
                          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                           CACHE_DIRECTORY_NAME"/new.ndx"), "wb");
                    // les deux doivent être ouvrables
