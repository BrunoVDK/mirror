...
int hts_record_link(httrackp * opt,
                    ...
                    const char *ref_address, const char *ref_file,
                    const char *codebase) {
    const int success = hts_record_link_(opt, address, file, save, ref_address, ref_file, codebase);
    if (!success) {
        hts_log_print(opt, LOG_PANIC, "Too many links (links=%ld, limit=%ld)", (long int) heap_top_index(), (long int) opt->maxlink);
        hts_log_print(opt, LOG_INFO, "To avoid that: use #L option for more links (example: -#L1000000, or -#L0 to disable)");
    }
    return success;
}
...
// MIRROR_ADAPTATION : HERE BELOW (in usercommand(...) -- Added 0 size but commented out index name as to prevent it from notifying project -- used to be 'fconcat(OPT_GET_BUFF(opt),OPT_GET_BUFF_SIZE(opt),StringBuff(opt->path_html_utf8),"index.html")'
...
makeindex_fp=NULL; \
usercommand(opt,0,NULL,NULL,NULL,NULL,-1);  \
} \
} \
...
int httpmirror(char *url1, httrackp * opt) {
    ...
    // MIRROR_ADAPTATION START : Changed global to local variables
    memset(&opt->stats, 0, sizeof(opt->stats));
    opt->stats.stat_timestart = time_local();
    //istat_timestart=stat_timestart;
    opt->stats.istat_timestart[0] = opt->stats.istat_timestart[1] = mtime_local();
    /* reset stats */
    opt->stats.HTS_TOTAL_RECV = 0;
    opt->stats.istat_bytes[0] = opt->stats.istat_bytes[1] = 0;
    if (opt->shell) {
        last_info_shell = opt->stats.stat_timestart;
    }
    if ((opt->makestat) || (opt->maketrack)) {
        makestat_time = opt->stats.stat_timestart;
    }
    // MIRROR_ADAPTATION END : Changed global to local variables
    ...
    usercommand(opt, opt->sys_com_exec, StringBuff(opt->sys_com), "", "", "", 0); // MIRROR_ADAPTATION : Added 0 size
    ...
    // copier adresse(s) dans liste des adresses
    {
        ...
        heap_top()->parent = 0; // MIRROR_ADAPTATION : Set parent to zero
    ...
    if (opt->makestat) {
        makestat_fp =
        fopen(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), STATS_FILE_NAME".txt"), "wb");
        if (makestat_fp != NULL) {
    ...
    if (opt->maketrack) {
        maketrack_fp =
        fopen(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), TRACK_FILE_NAME".txt"), "wb");
        ...
        if (!error) {
            ...
            if (heap(ptr)) {
                while(ptr < opt->lien_tot
                    ...
                    // remove from stats
                    if (heap(ptr)->pass2 == -1) {
                        opt->stats.stat_background--; // MIRROR_ADAPTATION : Changed global to local variables
                    }
                    ptr++;
                }
                // We're done!
...
#undef CH_ADD_RNG2
        ...
        if (!error) {
...
#endif
                            usercommand(opt, 0, NULL, fconv(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), tempo), "", "", -1); // MIRROR_ADAPTATION : Added -1 size
                        }
            ...
            /* Post-processing */
            if (fexist(savename())) {
                usercommand(opt, 0, NULL, savename(), urladr(), urlfil(), -1);  // MIRROR_ADAPTATION : Added -1 (undetermined) size
            }
    ...
    /*
     updating-a-remotely-deleted-website hack
     not much data transferred, no data saved
     <no files successfulyl saved>
     we assume that something was bad (no connection)
     just backup old cache and restore everything
     */
    // MIRROR_ADAPTATION START : Changed global to local variables
    if ((opt->stats.stat_files <= 0)
        && (opt->stats.HTS_TOTAL_RECV < 32768)      /* should be fine */
        ) {
        ...
        if ((fexist
             (fconcat
              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/old.dat")))
            &&
            (fexist
             (fconcat
              (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
               CACHE_DIRECTORY_NAME"/old.ndx")))) {
                  remove(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/new.dat"));
                  remove(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/new.ndx"));
                  remove(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/new.lst"));
                  remove(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/new.txt"));
                  rename(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/old.dat"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                   StringBuff(opt->path_log),
                                                                   CACHE_DIRECTORY_NAME"/new.dat"));
                  rename(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/old.ndx"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                   StringBuff(opt->path_log),
                                                                   CACHE_DIRECTORY_NAME"/new.ndx"));
                  rename(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/old.lst"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                   StringBuff(opt->path_log),
                                                                   CACHE_DIRECTORY_NAME"/new.lst"));
                  rename(fconcat
                         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                          CACHE_DIRECTORY_NAME"/old.txt"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                                   StringBuff(opt->path_log),
                                                                   CACHE_DIRECTORY_NAME"/new.txt"));
              }
    ...
    if (cache.lst) {
        ...
        if (opt->delete_old) {
            ...
            fopen(fconcat
                  (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                   CACHE_DIRECTORY_NAME"/old.lst"), "rb");
            if (old_lst) {
                const size_t sz =
                ...
                fopen(fconcat
                      (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                       CACHE_DIRECTORY_NAME"/new.lst"), "rb");
    ...
    if (opt->log != NULL) {
        ...
        (LLint) (opt->stats.HTS_TOTAL_RECV /
                 (max(1, time_local() - opt->stats.stat_timestart)));
        
        sec2str(htstime, time_local() - opt->stats.stat_timestart);
        ...
        infoupdated[0] = '\0';
        if (opt->is_update) {
            if (opt->stats.stat_updated_files > 0) {
                sprintf(infoupdated, ", %d files updated",
                        (int) opt->stats.stat_updated_files);
            } else {
        ...
        sprintf(finalInfo + strlen(finalInfo),
                ...
                (int) opt->stats.stat_files, (LLint) opt->stats.stat_bytes, infoupdated,
                (LLint) opt->stats.HTS_TOTAL_RECV, (LLint) n);
        
        if (opt->stats.total_packed > 0 && opt->stats.total_unpacked > 0) {
            int packed_ratio =
            (int) ((LLint) (opt->stats.total_packed * 100) / opt->stats.total_unpacked);
            sprintf(finalInfo + strlen(finalInfo),
                    ", " LLintP
                    " bytes transferred using HTTP compression in %d files, ratio %d%%",
                    (LLint) opt->stats.total_unpacked, opt->stats.total_packedfiles,
                    (int) packed_ratio);
        }
        if (!opt->nokeepalive && opt->stats.stat_sockid > 0
            && opt->stats.stat_nrequests > opt->stats.stat_sockid) {
            int rq = (opt->stats.stat_nrequests * 10) / opt->stats.stat_sockid;
    ...
    // ending
    usercommand(opt, 0, NULL, NULL, NULL, NULL, 0); // MIRROR_ADAPTATION : Added 0 size
 ...
 */
int engine_stats(httrackp *opt) { // MIRROR_ADAPTATION : Added 'opt' parameter
#if 0
...
#endif
    // MIRROR_ADAPTATION START : Changed global to local variables
    opt->stats.stat_nsocket = opt->stats.stat_errors = opt->stats.nbk = 0;
    opt->stats.nb = 0;
    if (opt->stats.HTS_TOTAL_RECV > 2048) {
        TStamp cdif = mtime_local();
        int i;
        
        for(i = 0; i < 2; i++) {
            if ((cdif - opt->stats.istat_timestart[i]) >= 2000) {
                TStamp dif;
                ...
                fflush(debug_fp);
#endif
                dif = cdif - opt->stats.istat_timestart[i];
                if ((TStamp) (dif / 1000) > 0) {
                    LLint byt = (opt->stats.HTS_TOTAL_RECV - opt->stats.istat_bytes[i]);
                    
                    opt->stats.rate = (LLint) ((TStamp) ((TStamp) byt / (dif / 1000)));
                    opt->stats.istat_idlasttimer = i;       // this timer recently sets the stats
                    //
                    opt->stats.istat_bytes[i] = opt->stats.HTS_TOTAL_RECV;
                    opt->stats.istat_timestart[i] = cdif;
                }
                return 1;               /* refreshed */
            }
        }
        ...
        if (opt->stats.istat_reference01 != opt->stats.istat_timestart[0]) {
            if ((cdif - opt->stats.istat_timestart[0]) >= 1000) {
#if 0
                fprintf(debug_fp, "resync timer 1\n");
                fflush(debug_fp);
#endif
                opt->stats.istat_bytes[1] = opt->stats.HTS_TOTAL_RECV;
                opt->stats.istat_timestart[1] = cdif;
                opt->stats.istat_reference01 = opt->stats.istat_timestart[0];
            }
        }
            ...
            if (heap(i)->adr) {
                ...
                if ((l > 0) && (l < 1020)) {    // sécurité
                    ...
                    if (opt->log != NULL) {
                        ...
                        hts_log_print(opt, LOG_WARNING, "Dump 1024 bytes (address %p): " LF "%s", heap(i)->adr, dmp);
                    }
                }
            } else {
...
static void postprocess_file(httrackp * opt, const char *save, const char *adr,
                             const char *fil);
void usercommand(httrackp * opt, int _exe, const char *_cmd, const char *file, const char *adr, const char *fil, LLint size) { // MIRROR_ADAPTATION : Added 'size' parameter
    ...
    if (file != NULL && strnotempty(file)) {
        RUN_CALLBACK4(opt, filesave, fil, adr, file, size); // MIRROR_ADAPTATION : Used to be 'RUN_CALLBACK1(opt, filesave, file);'
    }
...
int backlinks_done(const struct_back * sback,
                   ...
                   int ptr, httrackp *opt) { // MIRROR_ADAPTATION : Added 'opt' parameter
    int n = 0;
    ...
    // MIRROR_ADAPTATION : Changed global to local variables
    n += opt->stats.stat_background;
#endif
...
int back_pluggable_sockets_strict(struct_back * sback, httrackp * opt) {
    ...
    // connect limiter
    // MIRROR_ADAPTATION START : Changed global to local variables
    if (n > 0 && opt->maxconn > 0 && opt->stats.last_connect > 0) {
        TStamp opTime =
        opt->stats.last_request ? opt->stats.last_request : opt->stats.last_connect;
                ...
                if (!back_exist
                    ...
                    if (back_add
                        (sback, opt, cache, heap(p)->adr, heap(p)->fil, heap(p)->sav,
                         ...
                         heap(p)->testmode, heap(p)->parent) == -1) { // MIRROR_ADAPTATION : Added 'heap(p)->parent' parameter
                            hts_log_print(opt, LOG_DEBUG,
                ...
                if (ident_url_relatif(lien, urladr(), codebase, &afs.af) >= 0) { // reformage selon chemin
                    ...
                    if (!forbidden_url) {
                        ...
                        if (r != -1 && forbidden_url == 0 && IS_DELAYED_EXT(afs.save)
                            ...
                            r =
                            hts_wait_delayed(str, &afs, NULL, NULL, &former, &forbidden_url, -1); // MIRROR_ADAPTATION : Added -1 argument
                        }
                    ...
                    if (r != -1) {
                        ...
                        if (!dejafait) {
                            ...
                            heap_top()->parent = heap(ptr)->parent; // MIRROR_ADAPATION : Set parent variable
