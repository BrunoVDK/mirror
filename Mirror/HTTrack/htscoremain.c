...
static int hts_main_internal(int argc, char **argv, httrackp * opt) {
  ...
  // Create options
    // MIRROR_ADAPTATION : Changed '_DEBUG_HEAD' to 'opt->debug_head'
  opt->debug_head = 0;              // pas de debuggage en têtes
        ...
        if ((!fexist
             (fconcat
              ...
              StringBuff(opt->path_log),
               CACHE_DIRECTORY_NAME"/doit.log"))) || (argv_url > 0)) {
          if (!optinclude_file
              (fconcat
               (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
  ...
  if (fexist
      (fconcat
       (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
       StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/doit.log"))
      && (argv_url <= 0)) {
    FILE *fp =
      fopen(fconcat
            (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
            StringBuff(opt->path_log),
             CACHE_DIRECTORY_NAME"/doit.log"), "rb");
    if (fp) {
  ...
  if (!fexist
      (fconcat
       (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
       StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.zip"))) {
    if (fexist
        (fconcat
         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
         StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/old.zip"))) {
      rename(fconcat
             (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
             StringBuff(opt->path_log),
              CACHE_DIRECTORY_NAME"/old.zip"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                            StringBuff(opt->path_log),
                                            CACHE_DIRECTORY_NAME"/new.zip"));
    }
  } else
    if ((!fexist
         (fconcat
          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
          StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/new.dat")))
        ||
        (!fexist
         (fconcat
          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
    ...
    if ((fexist
         (fconcat
          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
          StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/old.dat")))
        &&
        (fexist
         (fconcat
          (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
  ...
  if (!opt->quiet) {
    if (fexist
        (fconcat
         (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
         StringBuff(opt->path_log),
          INPROGRESS_LOCK_NAME".lock"))) {
      /* Old cache */
      if ((fexist
           (fconcat
            (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
            StringBuff(opt->path_log),
             CACHE_DIRECTORY_NAME"/old.dat")))
          &&
          (fexist
           (fconcat
            (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
      ...
      if (argv[i][0] == '-') {
        if (argv[i][1] == '-') {        // --xxx
          if ((strfield2(argv[i] + 2, "clean")) || (strfield2(argv[i] + 2, "tide"))) {  // nettoyer
            ...
            if (fexist
                (fconcat
                 (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), LOG_FILE_NAME".txt")))
              remove(fconcat
                     (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                      LOG_FILE_NAME".txt"));
            if (fexist
                (fconcat
                 (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), ERR_FILE_NAME".txt")))
              remove(fconcat
                     (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                      ERR_FILE_NAME".txt"));
            if (fexist
            ...
            if (fexist
              ...
              remove(fconcat
                     (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                      INPROGRESS_LOCK_NAME".lock"));
            rmdir(fconcat
                  (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), CACHE_DIRECTORY_NAME));
            //
  ...
  /*
     if (argv_url==0) {
     //if ((fexist(fconcat(StringBuff(opt->path_log),CACHE_DIRECTORY_NAME"/new.dat"))) && (fexist(fconcat(StringBuff(opt->path_log),CACHE_DIRECTORY_NAME"/new.ndx")))) {  // il existe déja un cache précédent.. renommer
     if (fexist(fconcat(StringBuff(opt->path_log),CACHE_DIRECTORY_NAME"/doit.log"))) {    // un cache est présent
     ...
     fp=fopen(fconcat(StringBuff(opt->path_log),CACHE_DIRECTORY_NAME"/doit.log"),"rb");
     if (fp) {
  ...
  if (argv_url == 0) {
    ...
    if ((fexist
      ...
      if (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
        StringBuff(opt->path_log), CACHE_DIRECTORY_NAME"/doit.log"))) {        // un cache est présent
        if (x_argvblk != NULL) {
          int m;

          // établir mode - mode cache: 1 (cache valide) 2 (cache à vérifier)
          if (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
            StringBuff(opt->path_log), INPROGRESS_LOCK_NAME".lock"))) {  // cache prioritaire
            m = 1;
          } else {
            m = 2;
          }
          ...
          if (opt->quiet == 0) {        // sinon on continue automatiquement
            HT_REQUEST_START;
            HT_PRINT("A cache ("CACHE_DIRECTORY_NAME"/) has been found in the directory ");
    ...
    if (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
      StringBuff(opt->path_log), INPROGRESS_LOCK_NAME".lock"))) {        // fichier lock?
      //char s[32];
      ...
      if (opt->quiet == 0) {
        if ((fexist
          ...
          HT_REQUEST_START;
          HT_PRINT
            ("There is an index.html and a "CACHE_DIRECTORY_NAME" folder in the directory ");
          HT_PRINT(StringBuff(opt->path_log));
          HT_PRINT(LF
          ...
          case '%':{           // deuxième jeu d'options
                ...
                if ((na + 1 >= argc) || (argv[na + 1][0] == '-')) {
                  ...
                  if (fz < 0) {
                    ...
                    if (fp != NULL) {
                      ...
                      ensureUrlCapacity(url, url_sz, (int)(cl + fz + 8192));
              ...
              case 'F':        // footer id
                if ((na + 1 >= argc) || (argv[na + 1][0] == '-')) {
                  ...
                  printf
                    ("Example: -%%F \"<!-- Mirrored from %%s by Miroir, %%s -->\"\n");
                  htsmain_free();
                  return -1;
                } else {
                  na++;
              ...
              case 'H':        // debug headers
                opt->debug_head = 1; // MIRROR_ADAPTATION START : Changed global to local variables
                break;
              case 'O':
                printf
              ...
              case 'R':
                {
                  ...
                  if (unzRepair
                      (name,
                       fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/repair.zip"),
                       fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                               CACHE_DIRECTORY_NAME"/repair.tmp"), &repaired,
                       &repairedBytes) == Z_OK) {
                    unlink(name);
                    rename(fconcat
                           (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                            CACHE_DIRECTORY_NAME"/repair.zip"), name);
                    fprintf(stderr,
...
#undef FMT
  ...
  if (opt->cache) {
    if (fexist(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), INPROGRESS_LOCK_NAME".lock"))) {        // problemes..
      if (fexist
          (fconcat
           (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
            CACHE_DIRECTORY_NAME"/new.dat"))) {
        if (fexist
          ...
          if (fsize
              (fconcat
               (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                CACHE_DIRECTORY_NAME"/new.zip")) < 32768) {
            if (fsize
                (fconcat
                 (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                  CACHE_DIRECTORY_NAME"/old.zip")) > 65536) {
              if (fsize
                  (fconcat
                   (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                    CACHE_DIRECTORY_NAME"/old.zip")) > fsize(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                          StringBuff(opt->
                                                                     path_log),
                                                          CACHE_DIRECTORY_NAME"/new.zip")))
              {
        ...
        if (fexist
          ...
          if (fsize
              (fconcat
               (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                CACHE_DIRECTORY_NAME"/new.dat")) < 32768) {
            if (fsize
                (fconcat
                 (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                  CACHE_DIRECTORY_NAME"/old.dat")) > 65536) {
              if (fsize
                  (fconcat
                   (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                    CACHE_DIRECTORY_NAME"/old.dat")) > fsize(fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                          StringBuff(opt->
                                                                     path_log),
                                                          CACHE_DIRECTORY_NAME"/new.dat")))
              {
                ...
                rename(fconcat
                       (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                        CACHE_DIRECTORY_NAME"/old.ndx"), fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                                                      ...
                                                      CACHE_DIRECTORY_NAME"/new.ndx"));
                //} else {  // ne rien faire
                //  remove(CACHE_DIRECTORY_NAME"/old.dat");
                //  remove(CACHE_DIRECTORY_NAME"/old.ndx");
              }
            }
          }
        }
  ...
  if (opt->debug_head) {
    opt->ioinfo =
      fopen(fconcat
            (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log), IOINFO_NAME".txt"),
            "wb");
  }
...

  {
    ...
    // un petit lock-file pour indiquer un miroir en cours, ainsi qu'un éventuel fichier log
    {
      ...
      /* readme for information purpose */
      {
        FILE *fp =
          fopen(fconcat
                (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                 CACHE_DIRECTORY_NAME"/readme.txt"), "wb");
        if (fp) {
          fprintf(fp, "What's in this folder?" LF);
          fprintf(fp, "" LF);
          fprintf(fp,
                  "This folder ("CACHE_DIRECTORY_NAME") has been generated by WinHTTrack "
      ...
      strcpy(n_lock,
             fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                     INPROGRESS_LOCK_NAME".lock"));
      //sprintf(n_lock,fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),INPROGRESS_LOCK_NAME".lock"),n);
      /*do {
         if (!n)
         sprintf(n_lock,fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),INPROGRESS_LOCK_NAME".lock"),n);
         else
         sprintf(n_lock,fconcat(OPT_GET_BUFF(opt), StringBuff(opt->path_log),INPROGRESS_LOCK_NAME"%d.lock"),n);
         n++;
      ...
      if (opt->cache) {
        ...
        fp =
          fopen(fconcat
                (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_log),
                 CACHE_DIRECTORY_NAME"/doit.log"), "wb");
        if (fp) {
          for(i = 0 + 1; i < argc; i++) {
      ...
      if ((fp = fopen(n_lock, "wb")) != NULL) {
        ...
        fprintf(fp, LF);
        fprintf(fp,
                "To pause the engine: create an empty file named '"STOP_LOCK_NAME".lock'"
                LF);
#if USE_BEGINTHREAD
        fprintf(fp, "PID=%d\n", (int) getpid());
#ifndef _WIN32
      ...
      if (httrack_logmode) {
        printf("Mirror launched on %s by Miroir %s" LF, t,
               hts_get_version_info(opt));
        if (opt->wizard == 0) {
          printf
    ...
    /* Not or cleanly interrupted; erase CACHE_DIRECTORY_NAME/ref temporary directory */
