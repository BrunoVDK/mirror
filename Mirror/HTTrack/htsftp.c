...
int run_launch_ftp(FTPDownloadStruct * pStruct) {
  ...
  // connexion
  {
...

    {
      ...
      // --USER--
      get_ftp_line(opt, soc_ctl, line, sizeof(line), timeout);     // en tête  // MIRROR_ADAPTATION : Added 'opt' argument
      _CHECK_HALT_FTP;

      if (line[0] == '2') {     // ok, connecté
        strcpybuff(back->info, "login: user");
        snprintf(line, sizeof(line), "USER %s", user);
        send_line(opt, soc_ctl, line); // MIRROR_ADAPTATION : Added 'opt' argument
        get_ftp_line(opt, soc_ctl, line, sizeof(line), timeout); // MIRROR_ADAPTATION : Added 'opt' argument
        _CHECK_HALT_FTP;
        if ((line[0] == '3') || (line[0] == '2')) {
          ...
          if (line[0] == '2') { // ok
            send_line(opt, soc_ctl, "TYPE I"); // MIRROR_ADAPTATION : Added 'opt' argument
            get_ftp_line(opt, soc_ctl, line, sizeof(line), timeout); // MIRROR_ADAPTATION : Added 'opt' argument
            _CHECK_HALT_FTP;
            if (line[0] == '2') {
              // ok
        ...
        if (soc_dat != INVALID_SOCKET) {
          ...
          if (back->r.fp != NULL) {
            ...
            while((len > 0) && (!stop_ftp(back))) {
              ...
              // réception
              if (len) {
                len = (int)recv(soc_dat, buff, read_len, 0);
                if (len > 0) {
                  back->r.size += len;
                  opt->stats.HTS_TOTAL_RECV += len; // MIRROR_ADAPTATION : Changed from HTS_STAT.HTS_TOTAL_RECV += len;
                  RUN_CALLBACK4(opt, baseupdated, back->base_id, len, 0, 0); // MIRROR_ADAPTATION : Callback notification
                  if (back->r.fp) {
          ...
          if (back->r.statuscode != -1) {
            if (wait_socket_receive(soc_ctl, timeout_onfly) > 0) {
              ...
              get_ftp_line(opt, soc_ctl, line, sizeof(line), timeout);
              if (line[0] == '2') {     // OK
    ...
    send_line(opt, soc_ctl, "QUIT"); // bye bye  // MIRROR_ADAPTATION : Added 'opt' argument
    get_ftp_line(opt, soc_ctl, NULL, 0, timeout); // MIRROR_ADAPTATION : Added 'opt' argument
#ifdef _WIN32
    closesocket(soc_ctl);
#else
    close(soc_ctl);
...
int send_line(httrackp *opt, T_SOC soc, const char *data) {
  char BIGSTK line[1024];

    // MIRROR_ADAPTATION START : Added 'debug_head' and 'ioinfo' mappings (used to be global variables
  if (opt->debug_head) {
    if (opt->ioinfo) {
      fprintf(opt->ioinfo, "---> %s\x0d\x0a", data);
      fflush(opt->ioinfo);
    }
      // MIRROR_ADAPTATION END : Added 'debug_head' and 'ioinfo' mappings (used to be global variables
  }
#if FTP_DEBUG
  if (dd == NULL)
    dd = fopen("toto.txt", "w");
...
int get_ftp_line(httrackp *opt, T_SOC soc, char *ptrline, size_t line_size, int timeout) {
  ...
  do {
    ...
    case 1:
        opt->stats.HTS_TOTAL_RECV += 1; // MIRROR_ADAPTATION : Used to be HTS_STAT.HTS_TOTAL_RECV += 1;
      if ((b != 10) && (b != 13))
        data[i++] = b;
      break;
    default:
