...
int http_sendhead(httrackp * opt, t_cookie * cookie, int mode,
    ...
    // Envoi
    opt->stats.last_request = mtime_local(); // MIRROR_ADAPTATION START : Changed global to local variables
...
// MIROIR_ADAPATION : Added 'opt' and 'base_id' parameters
LLint http_xfread1(htsblk * r, int bufl, httrackp *opt, unsigned long base_id) {
    int nl = -1;
        ...
        if (!r->is_write) {         // stocker en mémoire
            if (r->totalsize >= 0) {  // totalsize déterminé ET ALLOUE
                ...
                if (r->adr != NULL) {
                    ...
                    // MIROIR_ADAPTATION : Added base_id argument
                    nl = req_size > 0 ? hts_read(r, r->adr + ((int) r->size), (int) req_size, opt, base_id) : 0;        /* NO 32 bit overlow possible here (no 4GB html!) */
                    // nouvelle taille
                    if (nl >= 0)
                        r->size += nl;
                ...
                if (r->adr != NULL) {
                    // lecture
                    // MIRROR_ADAPTATION : Added 'opt' and 'base_id' parameters
                    nl = hts_read(r, r->adr + (int) r->size, bufl, opt, base_id);
                    if (nl > 0) {
                        // resize
            ...
            if (buff != NULL) {
                // lecture
                // MIRROR_ADAPTATION : Added 'opt' and 'base_id' parameters
                nl = hts_read(r, buff, bufl, opt, base_id);
                // nouvelle taille
                if (nl > 0) {
                    r->size += nl;
        ...
        do {
            ...
            if (r->adr != NULL) {
                if (r->size < 8190) {
                    // lecture
                    // MIRROR_ADAPTATION : Added 'opt' and 'base_id' parameters
                    nl = hts_read(r, r->adr + r->size, 1, opt, base_id);
                    if (nl > 0) {
                        // exit if:
...
htsblk http_test(httrackp * opt, const char *adr, const char *fil, char *loc, unsigned long base_id) {
    T_SOC soc;
    htsblk retour;
    ...
    if (soc != INVALID_SOCKET) {
        ...
        do {
            // MIRROR_ADAPTATION : Added 'opt' and 'base_id' arguments
            if (http_xfread1(&retour, 0, opt, base_id) < 0)
                e = 1;
            else {
                if (retour.adr != NULL) {
...
T_SOC newhttp(httrackp * opt, const char *_iadr, htsblk * retour, int port,
    ...
    if (strcmp(_iadr, "file://") != 0) {  /* non fichier */
        ...
        if (retour != NULL) {
            retour->debugid = opt->stats.stat_sockid++;// MIRROR_ADAPTATION START : Changed global to local variables
        }
#if HTS_WIDE_DEBUG
        DEBUG_W("socket()=%d\n" _(int) soc);
#endif
        ...
        printf("connect\n");
#endif
        opt->stats.last_connect = mtime_local();// MIRROR_ADAPTATION START : Changed global to local variables
...
int hts_read(htsblk * r, char *buff, int size, httrackp *opt, unsigned long base_id) {
    ...
    if (r->is_file) {
...
#endif
            retour = (int)recv(r->soc, buff, size, 0);
            if (retour == 0) {
                retour = READ_EOF;
            } else if (retour < 0) {
                retour = READ_ERROR;
        ...
        if (retour > 0) {             // compter flux entrant
            opt->stats.HTS_TOTAL_RECV += retour; // MIRROR_ADAPTATION : Changed from HTS_STAT.HTS_TOTAL_RECV += retour;
            RUN_CALLBACK4(opt, baseupdated, base_id, retour, 0, 0); // MIRROR_ADAPTATION : Added callback call to 'baseupdated'
        }
#if HTS_USEOPENSSL
    }
#endif
...
HTSEXT_API void hts_set_log_vprint_callback(void (*callback)(httrackp * opt, int type, const char *format, va_list args)) {
...
HTSEXT_API void hts_log_vprint(httrackp * opt, int type, const char *format, va_list args) {
    ...
    //if (hts_log_print_callback != NULL) {
        ...
        RUN_CALLBACK3(opt, log, type, format, args_copy); // MIRROR_ADAPTATION : Added Callback call to replace callback below
        // hts_log_print_callback(opt, type, format, args_copy); // MIRROR_ADAPTATION : Changed 'args' parameter to 'args_copy' (seems to be an error)
        va_end(args_copy);
    // }
    if (opt != NULL && opt->log != NULL) {
        ...
        // MIRROR_ADAPTATION : Commented lines below (should interfere with log writing too, if set as an option)
...
HTSEXT_API httrackp *hts_create_opt(void) {
    ...
    opt->state.hts_addurl_identifier = 0; // MIRROR_ADAPTATION - Initialize identifier of ProjectURL to be added
...
static void __cdecl htsdefault_filesave(t_hts_callbackarg * carg, httrackp * opt, const char *file, const char *adr, const char *save, int size) {  // MIRROR_ADAPTATION : Added 'adr', 'save' and size parameters
}
...
static void __cdecl htsdefault_log(t_hts_callbackarg * carg, httrackp * opt, int type, const char *format, va_list args) {} // MIROIR_ADAPTATION : Added log default callback
