...
/* Transfer rate // MIRROR_ADAPTATION : Added 'opt' parameter */ \
engine_stats(opt); \
/* Refresh various stats // MIRROR_ADAPTATION START : Changed global to local variables*/ \
...
/* MIRROR_ADAPTATION : Added 'opt' parameter */ \
opt->stats.nbk=backlinks_done(sback,opt->liens,opt->lien_tot,ptr,opt); \
opt->stats.nb=back_transferred(opt->stats.stat_bytes,sback); \
/* Check // MIRROR_ADAPTATION END (except here below : Changed global to local variables*/ \
{ \
if (!RUN_CALLBACK7(opt, loop, sback->lnk, sback->count,-1,ptr,opt->lien_tot,(int) (time_local()-opt->stats.stat_timestart),&opt->stats)) { \
return -1; \
} \
} \
} \
...
int url_savename(lien_adrfilsave *const afs, lien_adrfil *const former, const char *referer_adr, const char *referer_fil,  httrackp * opt, struct_back * sback, cache_back * cache, hash_struct * hash, int ptr, int numero_passe, const lien_back * headers) {
            ...
            if (!((opt->check_type == 1) && (fil[strlen(fil) - 1] == '/')))   // slash doit être html?
                if (opt->savename_delayed == 2 || (ishtest = ishtml(opt, fil)) < 0) {   // on ne sait pas si c'est un html ou un fichier..
                    ...
                    // test imposible dans le cache, faire une requête
                        ...
                        // MIRROR_ADAPTATION : Added back->base_id argument
                        if (back_add(sback, opt, cache, current.adr, current.fil, BACK_ADD_TEST, referer_adr, referer_fil, 1, back->base_id) != -1) {
                            ...
                            if (b >= 0) {
                                ...
                                do {
                                    ...
                                    if (!RUN_CALLBACK7
                                        (opt, loop, sback->lnk, sback->count, b, ptr, opt->lien_tot,
                                         (int) (time_local() - opt->stats.stat_timestart),
                                         &opt->stats)) {
                                            return -1;
                                                        ...
                                                        if ((strcmp(moved.adr, current.adr))
                                                            ...
                                                            if (strfield(mov_url, "ftp://")) {     // ftp, ok on arrête
                                                                ...
                                                                if (!get_test_request)
                                                                    ...
                                                                    hts_log_print(opt, LOG_WARNING, "Loop with HEAD request (during prefetch) at %s%s", current.adr, current.fil);
                                                                }
                                                                // Ajouter
                                                                URLSAVENAME_WAIT_FOR_AVAILABLE_SOCKET();
                                                                // MIRROR_ADAPTATION : Added base_id argument
                                                                if (back_add(sback, opt, cache, moved.adr, moved.fil, methode, referer_adr, referer_fil, 1, back->base_id) != -1) {        // OK
                                ...
                                if (has_been_moved) {
                                    ...
                                    return url_savename(afs, NULL, referer_adr, referer_fil, opt, sback, cache, hash, ptr, numero_passe, NULL);
                                }
                                // --- --- ---
