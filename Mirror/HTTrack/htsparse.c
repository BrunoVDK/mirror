...
// COPY IN HTSCORE.C
 // MIRROR_ADAPTATION : Added -1 size in usercommand(...) here below
...
"<!-- Mirror and index made by Miroir -->", \
tempo, /* EOF */ NULL \
); \
fflush(makeindex_fp); \
fclose(makeindex_fp);  /* à ne pas oublier sinon on passe une nuit blanche */  \
makeindex_fp=NULL; \
usercommand(opt,0,NULL,fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),  StringBuff(opt->path_html_utf8),"index.html"),"primary","primary",-1);  \
} \
} \
        ...
        if (!error) {
            ...
            *(r->adr + r->size) = '\0';
            ...
            do {
                ...
                if (!makeindex_done) {  // autorisation d'écrire un index
                    if (!detect_title) {
                        ...
                        if (p != 0) {
                            ...
                            if (StringNotEmpty(opt->footer) || opt->urlmode != 4) {   /* != preserve */
                                ...
                                if (emited_footer == 1 && strnotempty(r->charset)) {
                                    HT_ADD
                                    ("<!-- Added by Miroir --><meta http-equiv=\"content-type\" content=\"text/html;charset=");
                                    HT_ADD(r->charset);
                                    HT_ADD("\" /><!-- /Added by Miroir -->");
                                    HT_ADD(eol);
                                }
                            }
                        }
                ...
                //}
                    ...
                    if ((p > 0) || (valid_p)) {   // on a repéré un lien
                        ...
                        if ((nofollow) || (opt->state.stop && /* force follow not to lose previous cache data */ !opt->is_update))
                            p_nocatch = 1;
                        ...
                        while((is_space(*html) || (inscriptgen && html[0] == '\\' && is_space(html[1]))) && quote == '\0') {
                            if (!quote)
                                if ((*html == '\"') || (*html == '\'')) {
                                ...
                                if (afs.af.adr[0] != '\0') {
                                    ...
                                    if ((p_type != 2) && (p_type != -2)) {        // pas base href ou codebase
                                        ...
                                        if (forbidden_url != 1) {
                                            ...
                                            r_sv = url_savename(&afs, &former, heap(ptr)->adr, heap(ptr)->fil, opt, sback, cache, hash, ptr, numero_passe, NULL);
                                            
                                            if (strcmp(jump_identification_const(last_adr), jump_identification_const(afs.af.adr)) != 0) {       // a changé
                                                ...
                                                if ((ptr > 0) && (p_type != 2) && (p_type != -2)) {     // tester autorisations?
                                                    if (!p_nocatch) {
                                                        if (afs.af.adr[0] != '\0') {
                                                            hts_log_print(opt, LOG_DEBUG, "wizard moved link retest at %s%s..", afs.af.adr, afs.af.fil);
                                                            forbidden_url =
                                                            hts_acceptlink(opt, ptr, afs.af.adr, afs.af.fil, intag_name ? intag_name : NULL, intag_name ? tag_attr_start : NULL, &set_prio_to, &just_test_it);
                                    ...
                                    if (r_sv != -1 && p_type != 2 && p_type != -2 && forbidden_url == 0 && IS_DELAYED_EXT(afs.save) ) {
                                        
                                        time_t t;
                                        
                                        // pas d'erreur, on continue
                                        // MIRROR_ADAPTATION : Added heap(ptr)->parent
                                        r_sv =
                                        hts_wait_delayed(str, &afs, heap(ptr)->adr,
                                                         heap(ptr)->fil, &former,
                                                         &forbidden_url, heap(ptr)->parent);
                                                    ...
                                                    if (!opt->external) {
                                                        ...
                                                        if (patch_it) {
                                                            ...
                                                            if (verif_external(opt, cat_nb, 1)) {
                                                                FILE *fp =
                                                                filecreate(&opt->state.strc, fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_html_utf8), cat_name));
                                                                if (fp) {
                                                                    if (cat_data_len == 0) {      // texte
                                                                        verif_backblue(opt, StringBuff(opt-> path_html_utf8));
                                                                        fprintf(fp, "%s%s", "<!-- Created by Mirror -->" LF, cat_data);
                                                                    } else {      // data
                                                                        fwrite(cat_data, cat_data_len, 1, fp);
                                                                    }
                                                                    fclose(fp);
                                                                    usercommand(opt, 0, NULL, fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt), StringBuff(opt->path_html_utf8), cat_name), "", "",cat_data_len); // MIRROR_ADAPTATION : Added 'cat_data_len' as size
                                                                }
                                                            }
                                        ...
                                        if ((afs.af.adr[0] != '\0') && (p_type != 2) && (p_type != -2) && (forbidden_url != 1)) {  // si le fichier n'existe pas, ajouter à la liste
                                            ...
                                            if (opt->maxlink > 0 && opt->lien_tot + 1 >= opt->maxlink) {       // trop de liens!
                                                ...
                                                if (!dejafait) {
                                                                ...
                                                                if (ishtml(opt, afs.af.fil) != 0) {    // pas la peine pour des fichiers isolés
                                                                    if (checkrobots(_ROBOTS, afs.af.adr, "") != -1) {    // robots.txt ?
                                                                        ...
                                                                        if (checkrobots(_ROBOTS, afs.af.adr, "") == -1) {  // robots.txt ?
                                                                            ...
                                                                            // MIRROR_ADAPTATION : Added heap_top()->parent ...
                                                                            heap_top()->parent = heap(ptr)->parent;
                                                                            heap_top()->depth = 0;
                                                    ...
                                                    // enregistrer
                                                    if (!hts_record_link(opt, afs.af.adr, afs.af.fil, afs.save, former.adr, former.fil, codebase)) {
                                                        if ((opt->getmode & 1) && (ptr > 0)) {
                                                            if (fp) {
                                                                fclose(fp);
                                                                fp = NULL;
                                                    ...
                                                    // MIRROR_ADAPTATION : Added heap_top()->parent = heap(ptr)->parent;
                                                    heap_top()->parent = heap(ptr)->parent;
                                                    heap_top()->precedent = ptr;
                                                    // noter la priorité
                                                    if (!set_prio_to)
                                                    ...
                                                    if (!just_test_it) {
                                                        hts_log_print(opt, LOG_DEBUG, "OK, NOTE: %s%s -> %s", heap_top()->adr, heap_top()->fil, heap_top()->sav);
                                                    } else {
                                                        hts_log_print(opt, LOG_DEBUG, "OK, TEST: %s%s", heap_top()->adr, heap_top()->fil);
                                                    }
                                                    
                                                } else {        // if !dejafait
                                                    hts_log_print(opt, LOG_DEBUG, "link has already been recorded, cancelled: %s", afs.save);
                                                }
                ...
                if (opt->state._hts_in_html_poll) {
                    ...
                    back_wait(sback, opt, cache, opt->stats.stat_timestart); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats' (make global variable local)
                    back_fillmax(sback, opt, cache, ptr, numero_passe);
                    
                    // Transfer rate
                    engine_stats(opt); // MIRROR_ADAPTATION : Added 'opt' argument
                    
                    // Refresh various stats
                    // MIRROR_ADAPTATION START : Changed 'HTS_STAT' to 'opt->stats'
                    opt->stats.stat_nsocket = back_nsoc(sback);
                    opt->stats.stat_errors = fspc(opt, NULL, "error");
                    opt->stats.stat_warnings = fspc(opt, NULL, "warning");
                    opt->stats.stat_infos = fspc(opt, NULL, "info");
                    opt->stats.nbk = backlinks_done(sback, opt->liens, opt->lien_tot, ptr, opt);  // MIRROR_ADAPTATION : Added 'opt' argument
                    opt->stats.nb = back_transferred(opt->stats.stat_bytes, sback);
                    // MIRROR_ADAPTATION END : Changed 'HTS_STAT' to 'opt->stats'
                    
                    // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                    if (!RUN_CALLBACK7(opt, loop, sback->lnk, sback->count, 0, ptr, opt->lien_tot, (int) (time_local() - opt->stats.stat_timestart), &opt->stats)) {
                ...
                if (engine_stats(opt)) { // MIRROR_ADAPTATION : Added 'opt' argument
                    // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                    back_wait(sback, opt, cache, opt->stats.stat_timestart);
                    back_fillmax(sback, opt, cache, ptr, numero_passe);
                }
    ...
    if (!error) {
        ...
        if (HTTP_IS_REDIRECT(r->statuscode)) {
            ...
            
            {
                ...
                if ((reponse =
                    ...
                    if (get_it == 1) {
                        ...
                        hts_log_print(opt, LOG_WARNING, "Warning moved treated for %s%s (real one is %s%s)", urladr(), urlfil(), moved->adr, moved->fil);
                        ...
                        //xxc xxc
                        {
                            ...
                            if (url_savename(&savedmoved, NULL, heap(heap(ptr)->precedent)->adr, heap(heap(ptr)->precedent)->fil, opt, sback, cache, hash, ptr, numero_passe, NULL) != -1) {
                                if (hash_read(hash, savedmoved.save, NULL, HASH_STRUCT_FILENAME) < 0) {   // n'existe pas déja
                                    ...
                                    hts_log_print(opt, LOG_INFO, "Moving %s to an existing file %s", heap(ptr)->fil, urlfil());
                                }
            ...
            // erreur HTTP (ex: 404, not found)
                   ...
                   || (r->statuscode == HTTP_REQUESTED_RANGE_NOT_SATISFIABLE)) { // Precondition Failed, c'est à dire pour nous redemander TOUT le fichier
            if (fexist_utf8(heap(ptr)->sav)) {
                remove(heap(ptr)->sav);        // Eliminer
            } else {
                hts_log_print(opt, LOG_WARNING, "Unexpected 412/416 error (%s) for %s%s, '%s' could not be found on disk", r->msg, urladr(), urlfil(), heap(ptr)->sav != NULL ? heap(ptr)->sav : "");
            }
                ...
                if ((heap(ptr)->retry <= 0) || (!can_retry)) { // retry épuisés (ou retry impossible)
                    if ((opt->retry > 0) && (can_retry)) {
                        ...
                        if (r->statuscode == STATUSCODE_TEST_OK) {  // test OK
                            hts_log_print(opt, LOG_INFO, "Test OK at link %s%s (from %s%s)", urladr(), urlfil(), heap(heap(ptr)->precedent)->adr, heap(heap(ptr)->precedent)->fil);
                        } else {
...
void hts_mirror_process_user_interaction(htsmoduleStruct * str,
    ...
    // pause/lock files
    {
        ...
        // user pause lockfile : create PAUSE_LOCK_NAME.lock --> HTTrack will be paused
        if (fexist
            (fconcat
             (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
              StringBuff(opt->path_log), STOP_LOCK_NAME".lock"))) {
                 // remove lockfile
                 remove(fconcat
                        (OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),
                         StringBuff(opt->path_log), STOP_LOCK_NAME".lock"));
        ...
        if (opt->fragment > 0) {
            // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
            if ((opt->stats.stat_bytes - stat_fragment) > opt->fragment) {
                do_pause = 1;
            }
        }
        // pause?
        if (do_pause) {
            ...
            while(back_nsoc(sback) > 0) {     // attendre fin des transferts
                back_wait(sback, opt, cache, opt->stats.stat_timestart); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                Sleep(200);
                {
                    back_wait(sback, opt, cache, opt->stats.stat_timestart); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                    ...
                    if (!RUN_CALLBACK7
                        (opt, loop, sback->lnk, sback->count, b, ptr, opt->lien_tot,
                         (int) (time_local() - opt->stats.stat_timestart), &opt->stats)
            ...
            // structcheck_init(1);
            {
                ...
                if (fp) {
                    ...
                    fprintf(fp,
                            ...
                            "" LF "", (LLint) opt->stats.stat_bytes); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                    fclose(fp);
                }
            }
            // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
            stat_fragment = opt->stats.stat_bytes;
            /* Info for wrappers */
            hts_log_print(opt, LOG_INFO, "Engine: pause: %s",
                          fconcat(OPT_GET_BUFF(opt), OPT_GET_BUFF_SIZE(opt),  StringBuff(opt->path_log),
                                  PAUSE_LOCK_NAME".lock"));
    ...
    if (opt->state._hts_addurl) {
        ...
        while(*opt->state._hts_addurl) {
            ...
            if (ident_url_absolute(add_url, &add.af) >= 0) {
                ...
                // ----Ajout----
                // calculer lien et éventuellement modifier addresse/fichier
                if (url_savename(&add, NULL, NULL, NULL, opt, sback, cache, hash, ptr, numero_passe, NULL) != -1) {
                    ...
                    if (hash_read(hash, add.save, NULL, HASH_STRUCT_FILENAME) < 0) { // n'existe pas déja
                        ...
                        if (hts_record_link(opt, add.af.adr, add.af.fil, add.save, "", "", NULL)) {
                            ...
                            heap_top()->retry = opt->retry;
                            heap_top()->premier = heap_top_index();
                            // MIRROR_ADAPTATION : Added heap_top()->parent = opt->state._hts_addurl_identifier;
                            heap_top()->parent = opt->state.hts_addurl_identifier;
                            heap_top()->precedent = heap_top_index();
                            
                            hts_log_print(opt, LOG_INFO, "Link added by user: %s%s", add.af.adr, add.af.fil);
                        ...
                        hts_log_print(opt, LOG_NOTICE, "Existing link %s%s not added after user request", add.af.adr, add.af.fil);
                    }
...
#endif
        // MIRROR_ADAPTATION : Added heap(ptr)->parent argument
        if (back_add(sback, opt, cache, urladr(), urlfil(), savename(), heap(heap(ptr)->precedent)->adr, heap(heap(ptr)->precedent)->fil, heap(ptr)->testmode, heap(ptr)->parent) == -1) {
                ...
                if ((int) (l - makestat_time) >= 60) {
                    if (makestat_fp != NULL) {
                        ...
                        fprintf(makestat_fp,
                                ...
                                (int) ((opt->stats.HTS_TOTAL_RECV -
                                        *stre->makestat_total_) / (l - makestat_time)),
                                (LLint) opt->stats.HTS_TOTAL_RECV,
                                (int) opt->lien_tot - *stre->makestat_lnk_, (int) opt->lien_tot);
                        fflush(makestat_fp);
                        // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                        *stre->makestat_total_ = opt->stats.HTS_TOTAL_RECV;
                            ...
                            if ((back[i].status > 0)) {
                                if (strcmp(back[i].url_sav, s) == 0) {  // ok trouvé
                                    if (back[i].status != 1000) {
                                        ...
                                        printf("FOUND LINK TO CANCEL %s", back[i].url_sav);
                    ...
                    if ((tl - *stre->last_info_shell_) > 0) {     // toute les 1 sec
                        ...
                        *stre->last_info_shell_ = tl;
                        ...
                        if ((*stre->info_shell_) || a) {
                            ...
                            fprintf(fp, "TIME %d" LF, (int) (tl - opt->stats.stat_timestart)); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                            fprintf(fp, "TOTAL %d" LF, (int) opt->stats.stat_bytes); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                            fprintf(fp, "RATE %d" LF,
                                    (int) (opt->stats.HTS_TOTAL_RECV /
                                           (tl - opt->stats.stat_timestart))); // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
        ...
        for(loops = 0, continue_loop = 1;
            ...
            if (opt->delayed_cached) {
                ...
                if (back.r.statuscode == HTTP_OK && strnotempty(back.r.contenttype)) {  // cache found, and aswer is 'OK'
                    ...
                    afs->save[0] = '\0';
                    url_savename(afs, former, heap(ptr)->adr, heap(ptr)->fil, opt, sback, cache, hash, ptr, numero_passe, &back);
            ...
            // MIRROR_ADAPTATION : Added base_id argument
            if (back_add_if_not_exists(sback, opt, cache, afs->af.adr, afs->af.fil, afs->save, parent_adr, parent_fil, 0, base_id) != -1) {
                    ...
                    if (back[b].r.statuscode == STATUSCODE_INVALID && back[b].r.adr == NULL) {
                        ...
                        /* Recompute filename with MIME type */
                        afs->save[0] = '\0';
                        url_savename(afs, former, heap(ptr)->adr, heap(ptr)->fil, opt, sback, cache, hash, ptr, numero_passe, &delayed_back);
                    ...
                    do {
                        ...
                        // on est obligé d'appeler le shell pour le refresh..
                        {
                            ...
                            // Transfer rate
                            engine_stats(opt); // MIRROR_ADAPTATION : Added 'opt' parameter
                            ...
                            opt->stats.stat_warnings = fspc(opt, NULL, "warning");
                            opt->stats.stat_infos = fspc(opt, NULL, "info");
                            opt->stats.nbk = backlinks_done(sback, opt->liens, opt->lien_tot, ptr, opt); // MIRROR_ADAPTATION : Added 'opt' parameter
                            opt->stats.nb = back_transferred(opt->stats.stat_bytes, sback);
                            // MIRROR_ADAPTATION END : Changed 'HTS_STAT' to 'opt->stats'
                            
                            // MIRROR_ADAPTATION : Changed 'HTS_STAT' to 'opt->stats'
                            if (!RUN_CALLBACK7(opt, loop, sback->lnk, sback->count, b, ptr, opt->lien_tot, (int) (time_local() - opt->stats.stat_timestart), &opt->stats)) {
                                    return -1;
                        ...
                        /* Moved! */
                            ...
                            if (strnotempty(mov_url)) {   // location existe!
                                ...
                                if (ident_url_relatif(mov_url, afs->af.adr, afs->af.fil, &moved) >= 0) {
                                    hts_log_print(opt, LOG_DEBUG, "Redirect while resolving type: %s%s -> %s%s", afs->af.adr, afs->af.fil, moved.adr, moved.fil);
                                        ...
                                        // check explicit forbidden - don't follow 3xx in this case
                                        {
                                            ...
                                            if (hts_acceptlink(opt, ptr, moved.adr, moved.fil, NULL, NULL, &set_prio_to, NULL) == 1) {     /* forbidden */
                                                ...
                                                *forbidden_url = 1;       /* Forbidden! */
                                                hts_log_print(opt, LOG_DEBUG, "link forbidden because of redirect beyond the mirror scope at %s%s -> %s%s", afs->af.adr, afs->af.fil, moved.adr, moved.fil);
