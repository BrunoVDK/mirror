...
#define WAIT_FOR_AVAILABLE_SOCKET() do { \
  ...
  while(back_pluggable_sockets_strict(sback, opt) <= 0) { \
    ...
    /* Transfer rate  // MIRROR_ADAPTATION : Added 'opt' argument*/ \
    engine_stats(opt); \
    /* Refresh various stats // MIRROR_ADAPTATION START : Changed 'HTS_STAT' to 'opt->stats'*/ \
    ...
    /* Check // MIRROR_ADAPTATION END (except here below) : Changed 'HTS_STAT' to 'opt->stats' */ \
