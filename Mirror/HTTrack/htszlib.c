...
int hts_extract_meta(const char *path) {
    char catbuff[CATBUFF_SIZE];
    unzFile zFile = unzOpen(fconcat(catbuff, sizeof(catbuff), path, CACHE_DIRECTORY_NAME"/new.zip"));
    zipFile zFileOut = zipOpen(fconcat(catbuff, sizeof(catbuff), path, CACHE_DIRECTORY_NAME"/meta.zip"), 0);
